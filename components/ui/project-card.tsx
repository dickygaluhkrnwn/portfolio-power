...
export interface Project {
  ...
  desc: string;
  category: string;
  // UPDATE: techStack sekarang array of object
    ...
    >
      ...
      <div className="p-6 flex flex-col flex-grow relative z-20">
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {project.techStack.map((tech, i) => (
            ...
            >
              ...
              {/* FIX: Render tech.name, bukan object tech utuh */}
              {tech.name} 
            </span>
          ))}
        </div>
        ...
        <div className="flex items-center gap-3 mt-auto">
          {project.demoLink && (
            // Gunakan onClick preventDefault atau biarkan parent menangani navigasi
            // Tapi karena ini tombol eksternal, kita pakai <a> atau button dengan onClick window.open
            <Button 
              size="sm" 
              className="w-full bg-primary/10 text-primary hover:bg-primary hover:text-white border-primary/20"
              onClick={(e) => {
                e.stopPropagation(); // Mencegah klik card parent
