  ...
  return (
    <main className="min-h-screen bg-background text-foreground relative overflow-hidden">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        <div className="absolute top-[20%] left-[-10%] w-[300px] md:w-[400px] h-[300px] md:h-[400px] bg-accent/10 rounded-full blur-[80px] md:blur-[100px]" />
        <div className="absolute bottom-[20%] right-[-10%] w-[250px] md:w-[300px] h-[250px] md:h-[300px] bg-primary/10 rounded-full blur-[60px] md:blur-[80px]" />
      ...
      <div className="container-width pt-24 pb-16 md:pt-32 md:pb-20 relative z-10">
        ...
        <div className="mb-20 md:mb-24">
          ...
          >
            <h2 className="font-heading text-2xl md:text-3xl font-bold mb-3 md:mb-4 flex items-center gap-3">
              <Briefcase className="text-accent w-6 h-6" /> Proven Track Record
            </h2>
            ...
            <div className="space-y-6 md:space-y-8">
              {featuredProjects.map((project, idx) => (
                <motion.div
                  ...
                  className="group relative bg-secondary/10 border border-white/5 rounded-2xl p-5 md:p-8 hover:border-primary/30 transition-colors"
                >
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
                    <div className="md:col-span-1 space-y-4">
                      <div className="space-y-2">
                        <span className="text-xs font-bold text-primary tracking-widest uppercase">Case Study</span>
                        <h3 className="text-xl md:text-2xl font-heading font-bold">{project.title}</h3>
                      ...
                      <Button 
                        variant="outline" 
                        size="sm" 
                        className="w-full mt-4 group-hover:bg-primary group-hover:text-white group-hover:border-primary transition-all min-h-[40px]"
        ...
        >
          <h2 className="font-heading text-2xl md:text-3xl font-bold mb-6 md:mb-8 flex items-center gap-3">
            <Cpu className="text-primary w-6 h-6" /> Technical Arsenal
          </h2>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 md:gap-4">
            {baseTechStack.map((tech, idx) => {
              ...
              return (
                <div 
                  key={idx}
                  className="flex items-center gap-3 p-3 md:p-4 rounded-xl bg-secondary/20 border border-white/5 hover:bg-secondary/40 transition-all cursor-default group"
                >
                  <div className="p-2 rounded-lg bg-black/40 text-foreground/80 group-hover:text-primary group-hover:scale-110 transition-all shrink-0">
                    {tech.icon}
                  </div>
                  <div>
        ...
        {/* --- EXPERIENCE TIMELINE (Responsive Fix) --- */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
        >
          <h2 className="font-heading text-2xl md:text-3xl font-bold mb-8 md:mb-12 flex items-center gap-3">
            <Terminal className="text-accent w-6 h-6" /> Professional Journey
          </h2>
          ...
          <div className="space-y-10 border-l-2 border-white/5 ml-4 sm:ml-6 pl-8 sm:pl-10 relative">
            ...
            {journeyItems.map((item, idx) => (
              <div key={item.id} className="relative group">
                {/* Timeline Icon Marker - Adjusted Position */}
                ...
                <div className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-2 mb-2">
                  ...
                  <span className="text-xs sm:text-sm text-muted-foreground font-medium">{item.company}</span>
                </div>
                
                <div className="inline-block bg-white/5 px-2 py-0.5 sm:px-3 sm:py-1 rounded-full text-[10px] sm:text-xs text-primary font-mono mb-3 sm:mb-4">
                  {item.year}
                </div>
